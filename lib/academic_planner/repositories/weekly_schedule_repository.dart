...
class WeeklyScheduleRepository {
  WeeklyScheduleRepository({AcademicDatabase? academicDatabase})
    : _academicDatabase = academicDatabase ?? AcademicDatabase.instance;

  final AcademicDatabase _academicDatabase;

  Future<Map<int, List<ScheduledClass>>> getWeek() async {
    ...
    final result = <int, List<ScheduledClass>>{
      for (var weekday = DateTime.monday; weekday <= DateTime.sunday; weekday++)
        weekday: <ScheduledClass>[],
    };
    ...
    for (final row in rows) {
      ...
      result[session.weekday]!.add(
        ScheduledClass(subject: subject, session: session),
      );
    }
